...
**Definition 2.53.** Let $\mathcal{X}$ and $\mathcal{Y}$ be V-categories. Define their *V-product*, or simply *product*, to be the V-category $\mathcal{X} \times \mathcal{Y}$ with:
...
_×V_ {V} X Y = record
  ...
  where
    ...
    interchange {a} {b} {c} {d} =
      trans associativity (
      trans (cong (a ⊗_) (sym associativity)) (
      trans (cong (λ z → a ⊗ (z ⊗ d)) symmetry) (
      trans (cong (a ⊗_) associativity)
            (sym associativity))))
    ...
    composition-proof : ∀ {xy₁ xy₂ xy₃ : X.Ob × Y.Ob} →
      ...
      in ((X.hom x₁ x₂ ⊗ Y.hom y₁ y₂) ⊗ (X.hom x₂ x₃ ⊗ Y.hom y₂ y₃))
         ≤ (X.hom x₁ x₃ ⊗ Y.hom y₁ y₃)
    composition-proof {x₁ , y₁} {x₂ , y₂} {x₃ , y₃} =
      begin
        (X.hom x₁ x₂ ⊗ Y.hom y₁ y₂) ⊗ (X.hom x₂ x₃ ⊗ Y.hom y₂ y₃)
      ≡⟨ interchange ⟩
        (X.hom x₁ x₂ ⊗ X.hom x₂ x₃) ⊗ (Y.hom y₁ y₂ ⊗ Y.hom y₂ y₃)
      ≤⟨ monotonicity X.composition Y.composition ⟩
        X.hom x₁ x₃ ⊗ Y.hom y₁ y₃
      ∎
```
...
The full proof strategy is:

1. Start with: $(X(x_1,x_2) \otimes Y(y_1,y_2)) \otimes (X(x_2,x_3) \otimes Y(y_2,y_3))$

2. Use associativity and symmetry to rearrange to:
   $(X(x_1,x_2) \otimes X(x_2,x_3)) \otimes (Y(y_1,y_2) \otimes Y(y_2,y_3))$

3. Apply monotonicity with $\mathcal{X}$'s composition and $\mathcal{Y}$'s composition:
   $X(x_1,x_2) \otimes X(x_2,x_3) \leq X(x_1,x_3)$ and
   $Y(y_1,y_2) \otimes Y(y_2,y_3) \leq Y(y_1,y_3)$

4. Get: $X(x_1,x_3) \otimes Y(y_1,y_3)$

The rearrangement in step 2 is the **interchange law**, which we prove as a lemma using associativity and symmetry of $\otimes$.
